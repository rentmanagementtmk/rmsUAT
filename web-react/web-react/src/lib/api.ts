...
export async function apiGet<T = any>(params: Params, opts: { forceRefresh?: boolean } = {}): Promise<T> {
  ...
  let resp: Response;
  try {
    resp = await fetch(buildUrl(params, !!opts.forceRefresh), opts.forceRefresh ? { cache: 'reload' } : {});
  } catch (err) {
    debugLog('api-fetch-error', { action: params.action, tokenTail, message: (err as Error).message });
    throw err;
  }
  if (!resp.ok) {
...
export async function apiPost<T = any>(body: Record<string, unknown>): Promise<T> {
  ...
  try {
    ...
    debugLog('api-fetch-error', { action, tokenTail, message: (err as Error).message });
    throw err;
  }
  if (!resp.ok) {
